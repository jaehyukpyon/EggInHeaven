...
<link rel="stylesheet"
   href="${pageContext.request.contextPath}/resources/css/commu_list.css">
<link rel="stylesheet"
...
<body>
            ...
            <div class="search_box">
               ...
               <div class="search_field">
                  <input name="search_word" type="text" class="input"
                     placeholder="Search" value="${search_word}"> <i
                     class="fas fa-search"></i> <input type="hidden"
                     name="search_field" value="WSC" id="c">
               </div>
            </div>
         </div>
      </form>
      ...
      <c:if test="${listcount > 0 }">
               ...
               <c:forEach var="c" items="${commulist}">
                  <tr>
                     ...
                     <c:if test="${!empty c.commu_img}">
                        <%-- 사진첨부한 경우 --%>
                        <td style="width: 20%; float: right"><img
                           src="../upload${c.commu_img}" alt="파일첨부"></td>
                     </c:if>
                     <c:if test="${empty c.commu_img}">
                        <%-- 파일첨부 하지 않은 경우 --%>
                        <td></td>
                     ...
                     <td width="40%">
                        <%--제목 --%> <a href="detail?num=${c.commu_num}"> 
                        <c:out value="${c.commu_subject}" /> 
                        <span class="gray small" style="color: #007bff; font-weight: bold">&nbsp;(<c:out value="${c.cnt}" />)
                        </span>
                     </a>
                     </td>
         ...
         <div class="center-block">
            <ul class="pagination justify-content-center">
               <c:if test="${page <= 1}">
                  <li class="page-item"><a class="page-link gray">&laquo;</a></li>
               </c:if>
               <c:if test="${page > 1 }">
               ...
               <c:if test="${page >= maxpage}">
                  <li class="page-item"><a class="page-link gray">&raquo;</a></li>
               </c:if>
               <c:if test="${page < maxpage }">
                  ...
                  <li class="page-item"><a href="${next}" class="page-link">&raquo;</a>
                  </li>
               </c:if>
            </ul>

            <button id="write" type="button"
               class="btn btn-primary align-bottom mr-1 float-right write">글 쓰 기</button>
   ...
   <c:if test="${listcount == 0 && empty search_word}">
      <b>등록된 게시글이 없습니다.</b>
      <button id="write" type="button" style="margin-top: 300px"
         class="btn btn-primary align-bottom mr-1 float-right write">글 쓰 기</button>
   ...
   <c:if test="${listcount == 0 && !empty search_word}">
      ...
      <button id="write" type="button" style="margin-top: 300px"
         class="btn btn-primary align-bottom mr-1 float-right write">글 쓰 기</button>
      <button type="button" style="margin-top: 300px"
         class="btn btn-outline-primary float-right" onClick="history.go(-1)">돌아가기</button>
   </c:if>
